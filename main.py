# v2.5_long_real.py — TENDÊNCIA REAL + 4h + DIAS AUTO — CORRIGIDO PORTA
...
@app.route("/")
def home():
    return f"V2.5 LONGO REAL + 4h | {now_br()} BR", 200
...
async def main_loop():
    async with aiohttp.ClientSession() as session:
        await tg(session, f"<b>{VERSION} ATIVO NO RENDER</b>\n4h + Dias Auto\n{now_br()} BR")
        while True:
            try:
